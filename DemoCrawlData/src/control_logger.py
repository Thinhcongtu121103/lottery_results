class ControlLogger:
    ...
    def update_log(self, id, filename, status, last_step, total_record=None):
        ...
        SET filename = %s, status = %s, last_step = %s, extract_time = NOW(), total_record = %s
        WHERE id = %s
        """
        # Nếu không có total_record, truyền NULL cho trường này
        values = (filename, status, last_step, total_record if total_record is not None else None, id)
